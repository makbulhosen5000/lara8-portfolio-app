    ...
    <div class="head bg-muted">
        <div class="card-body ">
            ...
            <div class="col-md-12  d-flex justify-content-between align-items-center">
                  ...
                  @endif
                  <a href="{{route('resumes.recent.project.view')}}" class="btn btn-warning text-dark"> <i class="fa fa-list"></i> Recent Project List</a>
                </div>
            </div>
        </div>
    </div>
        ...
        <form action="{{(@$editData)?route('resumes.recent.project.update',$editData->id):route('resumes.recent.project.store')}} " method="POST" enctype="multipart/form-data">
              @csrf
                ...
                <div class="form-group">
                    ...
                    <textarea type="text" id="" cols="54" rows="10" name="description" required>{{@$editData->description}}</textarea>
                   
                    <font style="color:red">{{($errors->has('description'))?($errors->first('description')):''}} </font>
                </div>
                <div class="form-group">
                    <label for="my-input">URL</label>
                    <input id="my-input" class="form-control" type="text" name="url" placeholder="Enter Your Project URL" value="{{@$editData->url}}" required>
                    <font style="color:red">{{($errors->has('url'))?($errors->first('url')):''}} </font>
                </div>
                <div class="form-group">
                    <label for="my-input">Image</label>
                  <img src="{{(!empty($editData->image))?url('public/images/project/'.$editData->image):url('public/images/not_found_img.png')}}" id="image" style="width:530px;height:200px">
